"
Expansion: Don't want to modify original script:
exploration/04_trigger_explore.R , as that was used for slides. Therefore,
going to tinker around here.
...
df_snow_doy <- readr$read_csv("data/modis_first_day_no_snow.csv")
...
df_asi_wy_2000
df_snow_doy_classified <- df_snow_doy |>
  dplyr$arrange(calDoy) |>
  dplyr$mutate(
    worst_year = dplyr$row_number() %in% c(1:5),
    indicator = "snow_melt_day"
  ) |>
  dplyr$left_join(
    df_val
    ) |>
  dplyr$mutate(
    tp = worst_year & asi_2000
  )
