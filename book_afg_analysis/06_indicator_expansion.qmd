...
quick look at ndsi params - later we just decide to go w/ mean
```{r}
lp_ndsi_params <- map(
  wt_set,
  \(wt_temp){
...
```{r}
# combining with patchwork like this looks terrible when rendered in html
# box::use(patchwork[...])
...
girafe(ggobj =lp_f1_combined$NO_WT)
...
same plot, but w/ mixed forecast indicator included
...
mostly scrap code and abandoned
one off snow checks
