---
title: Composite Indicator - Optimization Study
...
format:
  html:
    ...
    fig-dpi: 96       # Web-resolution is sufficient
execute:
  include: false
  echo: false
  warning: false
...
a.  maximize prediction accuracy/performance of end-of-season ASI
b.  minimize final variance across selected weights.
...
all_params_txt <- glue_collapse(
  unique(unlist(l_params)), sep = ", ",
  last = ", and "
)

```

The different combinations sets are composed of various combinations of `r {all_params_txt}`
...
#| include: false
ggsave(
  "book_afg_analysis/chap7_composite_indicator_performance_comparison_no_mixed.png",
  width = 5,
  height=5,
  units ="in"
  )
...
![](chap7_composite_indicator_performance_comparison_no_mixed.png){width="100%"}
...
plot_low_var_optimal(
  ldf_top3$gte_1984_w_mixed_forecast_obs,
  pal = pal_indicator_composition2,
  label_wt_id = F
)+
  coord_flip()
```
...
ggsave(
  "book_afg_analysis/chap7_composite_indicator_performance_comparison.png",
  width = 5,
  height=5,
  units ="in"
  )
...
![](chap7_composite_indicator_performance_comparison.png){width="100%"}
