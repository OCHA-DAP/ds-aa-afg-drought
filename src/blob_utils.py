...
def load_env_vars():
    ...
    """
    sas_token = os.getenv("DSCI_AZ_SAS_DEV")
    container_name = "projects"
    storage_account = os.getenv("DSCI_AZ_STORAGE_ACCOUNT")
...
def list_blobs(sas_token, container_name, storage_account, name_starts_with):
    base_url = f"https://{storage_account}.blob.core.windows.net"
    sas_url = f"{base_url}/{container_name}" f"?{sas_token}"
    container_client = ContainerClient.from_container_url(sas_url)
    container_contents = container_client.list_blobs(
        name_starts_with=name_starts_with
    )
    return [blob.name for blob in container_contents]
