...
df_univariate |>
    ...
    knitr::kable(
        digits = 3,
        caption = "Univariate predictor performance (bootstrap CV)",
...
The CDI approach derives weights from logistic regression coefficients. But we could also use the model's predicted probabilities directly. This section compares three trigger approaches:
...
comparison_table |>
    knitr::kable(
        ...
        caption = "Comparison of trigger approaches (in-sample)"
    )
```
...
df_apr_compare |>
    ...
    gt() |>
    tab_header(
        title = "Trigger Comparison Across All Years",
        subtitle = "Sorted by predicted probability"
    ) |>
...
**Model Selection**: April window logistic regression (F1 = `r round(apr_model$metrics$mean[3], 2)`)
...
**Trigger Performance** (historical):

- Precision: `r round(tp / (tp + fp) * 100, 0)`%
- Recall: `r round(tp / (tp + fn) * 100, 0)`%
- F1: `r round(2 * tp / (2 * tp + fp + fn), 2)`
