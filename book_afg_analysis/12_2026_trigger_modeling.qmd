...
### SEAS5-Only vs Multivariate Comparison

Given lasso's results, how does a simple SEAS5-only model compare to a multivariate model using all lasso-selected variables?
...
tibble(
    Model = c("SEAS5 only (tuned threshold)", "Lasso-selected multivariate (tuned)"),
    ...
    knitr::kable(
        digits = 3,
        caption = "March window: SEAS5 univariate vs multivariate (bootstrap CV, tuned thresholds)"
    )
```
...
The RP-based threshold is a **budget constraint**, not a performance optimization.

- **F1-optimized threshold** (`r best_prob_threshold`): Maximizes prediction accuracy
- **RP-based threshold** (75th percentile): Constrains expected trigger frequency to ~1 per `r RP_THRESHOLD` years

The RP-based approach may not maximize F1, but it provides predictable trigger frequency for humanitarian planning and budgeting.
:::
