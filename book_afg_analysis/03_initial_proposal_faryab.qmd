---
title: Initial Analysis - Trigger Proposal
subtitle: Faryab
format:
  html:
    toc: true
...
-   Forecasted:
    -   SEAS5 – cumulative predicted rainfall f or MAM predicted form Nov-March @johnson2019seas5
-   Observational:
    -   CHIRPS – Monthly rainfall: November - May
    -   NDSI – Monthly NDSI: November – May
...
The heatmap & rainfall time-series produced here are included in this [slide deck](https://docs.google.com/presentation/d/1pfqpEGx-MB_-1A8PiCnDmcOwifUQ-EXyTDuqQyBiw1c/edit#slide=id.g2fe5782e44e_0_45)
...
# blob processed data
df_chirps <- blob_connect$read_blob_file("DF_ADM1_CHIRPS", show_col_types = FALSE) |>
    dplyr$filter(
        ADM1_NAME == "Faryab"
    ) |>
    dplyr$group_by(
...
df_seas5 <- blob_connect$read_blob_file("DF_FARYAB_SEAS5", show_col_types = FALSE) |>
    dplyr$mutate(
        valid_month = lubridate$month(valid_date),
        valid_year = lubridate$year(valid_date)
    )

df_snow <- blob_connect$read_blob_file("DF_ADM1_MODIS_SNOW", show_col_types = FALSE) |>
    dplyr$filter(
        ADM1_NAME == "Faryab",
...
df_smi <- blob_connect$read_blob_file("DF_ADM1_SMI", skip = 1, show_col_types = FALSE) |>
...
# FAO GIEWS data
df_ndvi <- readr$read_csv(
    "https://eo.apps.fao.org/asis/data/country/AFG/GRAPH_NDVI_AGRI/ndvi_adm1_data.csv",
    show_col_types = FALSE
) |>
    dplyr$filter(Province == "Faryab")

df_vhi <- readr$read_csv("https://www.fao.org/giews/earthobservation/asis/data/country/AFG/MAP_NDVI_ANOMALY/DATA/vhi_adm1_dekad_data.csv", show_col_types = FALSE) |>
    dplyr$filter(Province == "Faryab")
df_asi <- readr$read_csv("https://www.fao.org/giews/earthobservation/asis/data/country/AFG/MAP_ASI/DATA/ASI_Dekad_Season1_data.csv", show_col_types = FALSE) |>
    dplyr$filter(Province == "Faryab")
...
# Production data
df_prod_init <- blob_connect$read_blob_file("DF_PRODUCTION_DATA", skip = 3, .name_repair = "unique_quiet")
...
# CHIRPS (from 2000)
df_chirps_from_2000_mam_perf <- df_chirps_wy_2000 |>
    dplyr$mutate(
        worst_year = TRUE
    ) |>
    tidyr$complete(
...
This is only an initial exploration of when we could potential activate, but needs to be guided by the crisis timeline and proposed operations.

1.  **Fill out crisis timeline**, proposed activities, and clear framework of when triggers would need to be available

2.  **Compare this to trigger performance** and proposal to decide when to trigger (e.g. look at overall activation rates, activation points, thresholds)

3.  **Design monitoring framework,** ideally performed by OCHA Afghanistan with support from the Centre for Humanitarian Data
