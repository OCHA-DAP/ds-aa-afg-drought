...
clean_env_df <-  function(df,indicator){
  df |> 
    clean_names() |> 
    rename(
      !!indicator := value
    ) |> 
    select(matches("adm\\d"),date, indicator)
}

fps <- proj_paths() # convenience func to load paths


fps_ndvi <- str_subset(list.files(fps$PUB_PROCESSED_AF),"^modis_ndvi_\\d.*\\d.csv")

df_ndvi_adm2 <-  read_csv(fps$ADM2_NDVI_WFP_HDX) |> 
  rhdx:::strip_hxl() |> 
  clean_names()
...
df_chirps <- read_csv(fps$ADM1_CHIRPS_MONTHLY) |> 
  clean_env_df("rainfall")


# snow frac
df_snow <- read_csv(fps$ADM1_MODIS_SNOW) |> 
    filter(
        parameter=="NDSI_Snow_Cover_mean"
      ) |> 
  clean_env_df("snow_frac")

# ndvi
df_adm1_ndvi_modis <-  fps_ndvi |> 
  map(~read_csv(
    file.path(fps$PUB_PROCESSED_AFG,
              .x))
    )|> 
  list_rbind() |> 
  clean_env_df(
...
knitr::include_graphics(
  here::here("ndvi_yearly_composite_masked.gif")
  )
```
