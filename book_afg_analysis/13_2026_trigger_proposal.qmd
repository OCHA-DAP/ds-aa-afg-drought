...
### Baseline Sensitivity: 1991 vs 1984 {#sec-baseline-sensitivity}
...
```{r}
#| label: baseline-comparison-table
...
```{r}
#| label: seas5-threshold-timeseries
#| fig-height: 5
#| fig-width: 8
...
## Monitoring Artifacts {#sec-monitoring-artifacts}

Export distribution params (mu/sigma) and historical CDI timeseries for the W2 monitoring pipeline. Run once, then set `eval: false`.

```{r}
#| label: compute-distribution-params
#| eval: false

# Load raw ERA5 (same source as data-raw/16_prepare_2026_model_feature_datasets.R)
df_era5_raw <- blob_read(
    name = "ds-aa-afg-drought/raw/vector/historical_era5_land_ndjfmam_lte2025.parquet",
    container = "projects"
) |>
    janitor::clean_names() |>
...
# ERA5 March indicators: snow_cover, total_precipitation_sum, soil moisture (avg layers)
# Baseline: 1984-2025 inclusive (must exclude 2026+ monitoring data)
df_era5_march <- df_era5_raw |>
    filter(
        lubridate::month(date) == 3,
        lubridate::year(date) >= 1984,
        lubridate::year(date) <= 2025
    ) |>
    mutate(
        parameter = if_else(
            stringr::str_detect(parameter, "volumetric_soil_water_layer"),
            "volumetric_soil_water_1m", parameter
        )
    ) |>
    group_by(adm1_name, parameter, date, shape_area) |>
    summarise(value = mean(value), .groups = "drop") |>
    filter(parameter %in% c("snow_cover", "volumetric_soil_water_1m", "total_precipitation_sum"))

# Area-weighted regional merge
df_era5_merged <- df_era5_march |>
    group_by(parameter, date) |>
    summarise(value = weighted.mean(value, w = shape_area), .groups = "drop") |>
    filter(lubridate::year(date) >= 1984)

# Load FAO ASI & VHI (same URLs as R/load_funcs.R)
df_fao_raw <- bind_rows(
    readr::read_csv(
        "https://www.fao.org/giews/earthobservation/asis/data/country/AFG/MAP_ASI/DATA/ASI_Dekad_Season1_data.csv",
        show_col_types = FALSE
    ) |> mutate(parameter = "asi"),
    readr::read_csv(
        "https://www.fao.org/giews/earthobservation/asis/data/country/AFG/MAP_NDVI_ANOMALY/DATA/vhi_adm1_dekad_data.csv",
        show_col_types = FALSE
    ) |> mutate(parameter = "vhi")
) |>
    janitor::clean_names() |>
    rename(value = data, adm1_name = province) |>
    filter(
...
df_fao_merged <- df_fao_raw |>
    filter(lubridate::year(date) >= 1984, lubridate::year(date) <= 2025) |>
    group_by(parameter, date) |>
    summarise(value = weighted.mean(value, w = shape_area), .groups = "drop")

# SEAS5 April-issued (lt0=Apr, lt1=May) — df_seas5_raw already loaded above
df_seas5_apr_may <- df_seas5_raw |>
    filter(
        lubridate::month(issued_date) == 4,
        leadtime %in% c(0, 1)
    ) |>
    mutate(
        parameter = glue("seas5 {lubridate::month(valid_date, abbr = TRUE, label = TRUE)}"),
        date = issued_date
    ) |>
    rename(adm1_name = name, value = mean) |>
    left_join(df_area, by = "adm1_name") |>
...
# Combine all and compute mu/sigma (1984-2025 baseline)
df_all_merged <- bind_rows(df_era5_merged, df_fao_merged, df_seas5_apr_may)

df_distribution_params <- df_all_merged |>
    group_by(parameter) |>
    summarise(mu = mean(value), sigma = sd(value), .groups = "drop")

blob_write(
    df = df_distribution_params,
    name = "ds-aa-afg-drought/monitoring_inputs/2026/cdi_distribution_params.parquet",
    stage = "dev",
    container = "projects"
)

cat("Distribution params saved:\n")
df_distribution_params |> knitr::kable(digits = 4)
```

```{r}
#| label: compute-historical-timeseries
#| eval: false

# Pivot April feature set to long format for monitoring plot
# df_apr has wide z-scored columns; cdi_weights has term + weight

cdi_features <- cdi_weights$term

df_historical_long <- df_apr |>
    select(pub_year, all_of(cdi_features)) |>
    pivot_longer(
        cols = all_of(cdi_features),
        names_to = "parameter",
        values_to = "zscore"
    )

# Add CDI composite (from df_historical which has calc_cdi output)
df_cdi_row <- df_historical |>
    transmute(pub_year = year, parameter = "cdi", zscore = cdi)

df_timeseries <- bind_rows(df_historical_long, df_cdi_row) |>
    rename(year = pub_year) |>
    arrange(year, parameter)

blob_write(
    df = df_timeseries,
    name = "ds-aa-afg-drought/monitoring_inputs/2026/cdi_historical_timeseries.parquet",
    stage = "dev",
    container = "projects"
)

cat(glue("Historical timeseries saved: {nrow(df_timeseries)} rows, {length(unique(df_timeseries$year))} years\n"))
cat(glue("Parameters: {paste(unique(df_timeseries$parameter), collapse = ', ')}\n"))
```

## Appendix: Activation History {#sec-activation-history}
...
```{r}
#| label: write-activation-history
#| eval: false
