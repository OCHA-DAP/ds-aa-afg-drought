...
**Note**: The above table uses the default P > 0.5 classification threshold. With imbalanced data (~24% drought), optimal thresholds are typically lower (0.25-0.35). See Appendix for tuned threshold analysis.
...
df_univariate |>
    ...
    labs(
        ...
        subtitle = "Default P > 0.5 threshold",
...
### March Window: Lasso Feature Selection

For the March window, most predictors represent February conditions (before the growing season). Physically, only the SEAS5-MAM forecast should be predictive of end-of-season drought. We use **Lasso regression** to test this - lasso shrinks irrelevant coefficients to exactly zero, performing automatic feature selection.
...
**Interpretation**: Variables with non-zero coefficients are selected by lasso as predictive. If only SEAS5-MAM (or very few variables) survive, this confirms that pre-growing season observations have limited predictive value for end-of-season drought.

### March Predictor Collinearity

The positive coefficient on `total_precipitation_sum` is unexpected. Let's check the correlation structure:

```{r}
#| label: march-correlation
#| fig-height: 6
#| fig-width: 7

# Correlation matrix for March predictors
mar_predictors <- df_mar |> select(-drought, -pub_year)

cor_mat <- cor(mar_predictors, use = "complete.obs")
...
# Specific correlation between precip variables
cor(mar_predictors$total_precipitation_sum, mar_predictors$precip_cumsum)
```

The high correlation between `total_precipitation_sum` and `precip_cumsum` (they overlap by construction) creates a suppressor effect. When both are in the model, their coefficients become unstable.

### Lasso-Selected Ridge Model (March)

A common approach: use lasso for feature selection, then fit ridge on selected variables. We exclude `asi` (dropped by lasso) and `total_precipitation_sum` (suppressor effect, redundant with `precip_cumsum`).

```{r}
#| label: march-lasso-ridge

# Variables selected by lasso, excluding the problematic ones
...
final_ridge_mar <- ridge_wf_mar |>
    finalize_workflow(best_penalty_mar) |>
    fit(data = df_mar)

# Extract coefficients
coefs_ridge_mar <- final_ridge_mar |>
    extract_fit_parsnip() |>
    tidy() |>
    filter(term != "(Intercept)") |>
    mutate(
...
coefs_ridge_mar |>
    ...
    knitr::kable(
        ...
        caption = "March window: Lasso-selected ridge coefficients"
    )
```

### Univariate SEAS5 Model (March)

How does the multivariate model compare to simply using the SEAS5 forecast alone?

```{r}
#| label: march-seas5-univariate

# Univariate SEAS5 model (standard GLM - no regularization needed with 1 predictor)
...
# Get metrics for lasso-selected ridge (with tuned threshold)
...
# Comparison table
tibble(
    Model = c("SEAS5 only (tuned)", "Lasso-selected ridge (tuned)"),
    Threshold = c(best_seas5_thresh$threshold, best_ridge_mar_thresh$threshold),
    F1 = c(best_seas5_thresh$f1, best_ridge_mar_thresh$f1),
    Precision = c(best_seas5_thresh$precision, best_ridge_mar_thresh$precision),
    Recall = c(best_seas5_thresh$recall, best_ridge_mar_thresh$recall)
) |>
    knitr::kable(
        digits = 3,
        caption = "March window: SEAS5 univariate vs lasso-selected multivariate (bootstrap CV, tuned thresholds)"
    )
```

**Interpretation**: If the univariate SEAS5 model performs similarly to the multivariate model, it suggests that February observational data adds limited value for the March trigger window - the seasonal forecast captures most of the predictive signal.
...
## Appendix: Tuned Threshold Univariate Analysis

The main univariate analysis uses default P > 0.5 threshold for fair comparison. Here we show performance with thresholds tuned per predictor to maximize F1. This represents a theoretical ceiling for each predictor's performance.
