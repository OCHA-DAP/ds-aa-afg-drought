...
```{r}
#| label: cerf-metrics-calculation
...
# Create metrics labels for top axis annotation
df_top_labels <- df_metrics |>
    mutate(
        metrics_label = paste0("P:", round(precision, 2), " R:", round(recall, 2), " F1:", round(f1, 2))
    ) |>
    select(indicator_label, metrics_label) |>
    add_row(indicator_label = "CERF Allocation", metrics_label = "(reference)")

# Get max year for positioning top labels
max_year <- max(df_heatmap$year)
...
ggplot(df_heatmap, aes(x = indicator, y = factor(year), fill = fill_group)) +
    ...
    labs(
        ...
        subtitle = "Metrics (P=Precision, R=Recall, F1) calculated from 2006-2025 for predicting CERF allocation\nRed = individual indicators, Green = joint triggers & CERF allocations",
        x = "Indicator",
        y = "Year"
    ) +
    theme_minimal() +
